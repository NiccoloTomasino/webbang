            ...
            <div className="mt-8 grid grid-cols-2 gap-x-4 gap-y-6 sm:mt-10 sm:gap-x-6 sm:gap-y-8">
              {stats.map((s, i) => (
                <Reveal key={s.label} delay={0.1 * i} className="border-l-2 border-bang pl-4 sm:pl-5">
                  <p className="whitespace-nowrap font-display text-[2.1rem] font-extrabold tracking-tight text-ink min-[400px]:text-4xl sm:text-5xl">
                    <Counter to={s.value} />
              ...
              <Reveal key={r.title} delay={i * 0.08} className={i % 2 === 1 ? "sm:mt-12" : ""}>
                <SpotlightCard className="h-full">
                  <div className="flex h-full flex-col p-6 sm:p-7">
                    <span className="mb-5 grid h-12 w-12 sm:mb-10 sm:h-14 sm:w-14 place-items-center rounded-2xl border border-ink/10 bg-ink/[0.04] text-bang-ink transition-all duration-500 group-hover:bg-bang group-hover:text-ink">
                      <r.icon className="h-6 w-6" />
                    </span>
                    <h3 className="font-display text-xl font-bold tracking-tight text-ink sm:text-2xl">{r.title}</h3>
