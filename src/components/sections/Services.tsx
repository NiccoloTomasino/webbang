    ...
    <section id="servizi" className="relative py-16 sm:py-28">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="mb-10 grid items-end gap-6 sm:mb-14 sm:gap-8 lg:grid-cols-2">
          ...
          <Reveal delay={0.2}>
            <p className="max-w-lg text-base leading-relaxed text-white/55 sm:text-lg lg:ml-auto">
              Non solo siti web. Siamo un&apos;<span className="text-white">agenzia digitale e di marketing</span>: costruiamo
        ...
        <div className="space-y-14 sm:space-y-20">
          {GROUPS.map((g, gi) => (
            <div key={g.id} id={g.id} className="scroll-mt-28">
              <Reveal>
                <div className="mb-5 flex flex-col gap-2 border-t border-white/[0.08] pt-6 sm:mb-7 sm:flex-row sm:items-end sm:justify-between sm:gap-6">
                  <div className="flex items-baseline gap-4">
                    <span className="font-mono text-sm text-bang">0{gi + 1}</span>
                    <div>
                      <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-white/40">{g.label}</p>
                      <h3 className="mt-1 font-display text-2xl font-bold tracking-tight text-white sm:text-3xl">{g.title}</h3>
                    </div>
                  </div>
                  <p className="max-w-sm pl-9 text-sm leading-relaxed text-white/45 sm:pl-0 sm:text-right">{g.text}</p>
                </div>
              </Reveal>
...
function ServiceBody({ service: s, index }: { service: Service; index: number }) {
  ...
  return (
    <div className={cn("flex h-full flex-col p-5 sm:p-7", big && "sm:p-9")}>
      <div className={cn("flex items-start justify-between", s.featured ? "mb-5 sm:mb-6" : "mb-4 sm:mb-6")}>
        <span
          className={cn(
            ...
            s.featured ? "h-12 w-12 bg-bang text-ink sm:h-14 sm:w-14" : "h-11 w-11 border border-white/10 bg-white/[0.04] text-bang sm:h-12 sm:w-12",
          )}
        >
      ...
      {s.featured && (
        <span className="mb-3 inline-flex w-fit rounded-full border border-bang/30 px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-widest text-bang">
          Specialità Webbang
        </span>
      )}

      <h3 className={cn("font-display font-bold tracking-tight text-white", big ? "text-[1.75rem] sm:text-4xl" : "text-xl sm:text-2xl")}>
        {s.title}
      </h3>
      <p className={cn("mt-2.5 leading-relaxed text-white/50 sm:mt-3", big ? "text-[15px] sm:text-lg" : "text-sm")}>{s.text}</p>
      ...
      <div className="mt-auto flex items-end justify-between gap-3 pt-5 sm:pt-6">
        <div className="flex flex-wrap gap-1.5">
          {s.tags.map((t) => (
            <span key={t} className="rounded-full bg-white/[0.05] px-2.5 py-1 text-[11px] text-white/50">
              {t}
            </span>
          ))}
        </div>
        <a
          ...
          className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-white/10 text-white/60 transition-all duration-300 group-hover:border-bang group-hover:bg-bang group-hover:text-ink"
        >
...
function MiniPipeline() {
  ...
  return (
    <div className="mt-6 grid grid-cols-3 gap-1.5 sm:mt-7 sm:gap-2.5">
      {cols.map((c, ci) => (
        <div key={c.name} className="min-w-0 rounded-xl border border-white/[0.06] bg-ink/60 p-1.5 sm:p-2.5">
          <p className="mb-2 truncate px-0.5 font-mono text-[9px] uppercase tracking-wider text-white/35 sm:text-[10px]">{c.name}</p>
            ...
            {c.cards.map((card, i) => (
              <motion.div
                ...
                className={cn(
                  ...
                  ci === 2 ? "bg-bang font-semibold text-ink" : "bg-white/[0.06] text-white/70",
                )}
              >
                {card}
              </motion.div>
        ...
        <div key={st.name} className="flex justify-center">
          <motion.div
            ...
            className={cn(
              ...
              i === 2 ? "bg-bang text-ink" : "border border-white/[0.06] bg-ink/60 text-white/70",
            )}
          >
