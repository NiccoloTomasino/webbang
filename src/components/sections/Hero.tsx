    ...
    >
      ...
      <div className="mx-auto grid max-w-7xl items-center gap-16 px-5 sm:px-8 lg:grid-cols-[1.2fr_1fr] lg:gap-10">
        ...
        <motion.div style={{ y: textY, opacity: fade }}>
          <motion.div
            ...
            className="mb-6 inline-flex items-center gap-2 rounded-full border border-ink/10 bg-ink/[0.04] py-1.5 pl-1.5 pr-4 text-xs text-ink/75 backdrop-blur sm:mb-7"
          >
          ...
          <h1 className="font-display text-[clamp(2.2rem,10.4vw,4.75rem)] lg:text-[clamp(3rem,5.6vw,4.75rem)] font-extrabold leading-[0.95] tracking-[-0.035em] text-ink">
            ...
            <span className="relative block h-[1.05em] overflow-hidden">
              <AnimatePresence mode="popLayout" initial={false}>
                <motion.span
                  key={WORDS[wordIndex]}
                  className="absolute left-0 top-0 block whitespace-nowrap mark-bang text-ink"
          ...
          <motion.p
            ...
            className="mt-6 max-w-xl text-base leading-relaxed text-ink/70 sm:mt-7 sm:text-lg"
          >
            Siamo l&apos;agenzia digitale che unisce <strong className="font-semibold text-ink">design premium</strong> e{" "}
            <strong className="font-semibold text-ink">software su misura</strong>: siti che convertono, CRM e web app
          ...
          <motion.ul
            ...
            className="mt-6 grid gap-2.5 text-[15px] text-ink/75 sm:text-sm"
          >
            {[
              ...
              >
                <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-bang/15">
                  <Check className="h-3 w-3 text-bang-ink" strokeWidth={3} />
                </span>
                {t}
              </motion.li>
            ))}
      ...
      <div className="mx-auto mt-14 max-w-7xl px-5 sm:px-8 lg:mt-20">
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.9, delay: 1.1, ease: EASE }}
          className="grid grid-cols-2 overflow-hidden rounded-3xl border border-ink/[0.08] bg-ink/[0.03] backdrop-blur md:grid-cols-4"
        >
          {stats.map((s, i) => (
            <div
              key={s.label}
              className="relative border-ink/[0.08] px-5 py-6 sm:px-6 sm:py-7 [&:not(:last-child)]:border-r max-md:[&:nth-child(2)]:border-r-0 max-md:[&:nth-child(-n+2)]:border-b"
            >
              <p className="whitespace-nowrap font-display text-[2rem] font-extrabold tracking-tight text-ink min-[400px]:text-4xl sm:text-5xl">
                <Counter to={s.value} duration={2 + i * 0.2} />
                <span className="text-bang-ink">{s.suffix}</span>
              </p>
              <p className="mt-1 text-xs font-medium uppercase tracking-wider text-ink/55">{s.label}</p>
            </div>
          ))}
        </motion.div>
      </div>
...
function HeroDashboard() {
  ...
  return (
    <div className="relative mx-auto max-w-[560px] pt-14 [transform-style:preserve-3d] sm:pt-0">
      {/* finestra principale */}
      <div className="relative overflow-hidden rounded-2xl border border-ink/10 bg-surface/90 shadow-[0_40px_100px_-35px_rgba(11,11,13,0.35)] backdrop-blur-xl">
        <div className="flex items-center gap-2 border-b border-ink/[0.08] px-4 py-3">
          ...
          <span className="ml-3 flex-1 truncate rounded-md bg-ink/[0.05] px-3 py-1 font-mono text-[10px] text-ink/55">
            app.tuaazienda.it/dashboard
          </span>
        </div>

        <div className="grid grid-cols-[1fr] min-[400px]:grid-cols-[52px_1fr]">
          <div className="hidden flex-col items-center gap-3 border-r border-ink/[0.08] py-4 min-[400px]:flex">
            {[0, 1, 2, 3, 4].map((i) => (
              <span
                key={i}
                className={`h-7 w-7 rounded-lg ${i === 0 ? "bg-bang" : "bg-ink/[0.05]"}`}
              />
            ))}
          </div>
          ...
          <div className="space-y-3 p-3 sm:p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-mono text-[10px] uppercase tracking-widest text-ink/50">Panoramica</p>
                <p className="whitespace-nowrap font-display text-base font-bold text-ink sm:text-lg">Buongiorno, Marco 👋</p>
              </div>
              <span className="whitespace-nowrap rounded-full bg-emerald-500/10 px-2.5 py-1 text-[10px] font-semibold text-emerald-700">
                ● Live
              </span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div key={k.l} className="rounded-xl border border-ink/[0.08] bg-ink/[0.04] p-2.5">
                  <p className="text-[10px] text-ink/55">{k.l}</p>
                  <p className="font-display text-base font-bold text-ink sm:text-lg">
                    <Counter to={k.v} duration={2.4} />
                    {k.s}
                  </p>
                  <p className="text-[10px] font-semibold text-bang-ink">{k.d}</p>
                </div>
              ))}
            </div>
            ...
            <div className="rounded-xl border border-ink/[0.08] bg-ink/[0.03] p-3">
              <div className="mb-2 flex items-center justify-between">
                <p className="text-[11px] font-semibold text-ink/75">Andamento vendite</p>
                <p className="font-mono text-[10px] text-ink/50">Ultimi 12 mesi</p>
              </div>
              <svg viewBox="0 0 300 90" className="h-24 w-full" aria-hidden>
                ...
                {[20, 45, 70].map((y) => (
                  <line key={y} x1="0" x2="300" y1={y} y2={y} stroke="rgba(11,11,13,0.07)" />
                ))}
                <motion.path
            ...
            <div className="grid grid-cols-5 items-end gap-1.5 rounded-xl border border-ink/[0.08] bg-ink/[0.03] p-3">
              {[55, 80, 40, 95, 70].map((h, i) => (
                <div key={i} className="flex flex-col items-center gap-1">
                  <div className="flex h-14 w-full items-end">
                    <motion.div
                      className={`w-full rounded-md ${i === 3 ? "bg-bang" : "bg-ink/12"}`}
                      initial={{ height: 0 }}
                      animate={{ height: `${h}%` }}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="block truncate text-[10px] text-ink/60">{n.sub}</span>
            </span>
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      >
        <p className="mb-2 font-mono text-[10px] uppercase tracking-widest text-ink/55">Attività di oggi</p>
        {["Controllo scadenze", "Invio preventivi", "Report settimanale"].map((t, i) => (
          <div key={t} className="flex items-center gap-2 py-1">
            <motion.span
              className="grid h-4 w-4 place-items-center rounded-[5px] border border-ink/16"
            ...
            <span className="text-[11px] text-ink/75">{t}</span>
          </div>
        ))}
      </motion.div>
    </div>
